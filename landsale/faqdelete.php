...
if (isset($_SESSION["id"])) {
    ...
    if ($registerResult && mysqli_num_rows($registerResult) > 0) {
        ...
        $deleteLandQuery = "DELETE FROM faq WHERE Fullname = '$fullname'";
        $deleteLandResult = mysqli_query($conn, $deleteLandQuery);
        if ($deleteLandResult) {
            // Additional actions after successful deletion
            echo "<script>alert('FAQ deleted successfully from the website.'); window.location.href = 'faq.php';</script>";
        } else {
